...
if(mod(n,30)==0 && jestF111 && n*DtR>7) 
    ...
    plot([n-1:n-2+Nopt+DelR]*DtR,Ur(n-1:n-2+Nopt+DelR),'g',...
        ...
        [n n]*DtR,[yn yn],'b.',[n n]*DtR,[ynp ynp],'r.');    
    axis('tight'); title(typReg);
    hold on; ax=axis; 
    if(n>8.5/DtR) if(n>12/DtR) ax(1)=12; else ax(1)=8; end, end %(n-200)*DtR; end
    axis(ax); 
    plot([n n]*DtR,ax(3:4),'k:',[n+DelR n+DelR]*DtR,ax(3:4),'r:')
    xlabel('U_g Yref_m ypl_k (ypl-vpr)_{k:} yn_{b.} ynp_{r.}')
    co=input(' Co dalej ?? <Ent> dalej; <0> 0KONIEC sledzenia; inne - dalej '); 
    if(~isempty(co)) jestF111=co; if(co==0) hold off; end, end
end