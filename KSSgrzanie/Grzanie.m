...
if(charKotla==1)
    ...
    global juzByleta Tob Kob rzads rzad rps KTTs KTq0 DtR;
    global wAqTqT Aq0 AqTg AqT KTsT KTsTzw Taus Mpow  Fvwz Fwconst;
    global ndrg ndrg;
    global przyr wagi nfig1 invh;
    global Unom X0;
    global hPom hEmp nTwg nTwg1 lX KSSlab PokazId jestF0111;
    KSSlab=0; PokazId=1; jestF0111=0;
...
cicho=1;
% ...... Liczymy opoznienia 
Lrg=30; Lrz=15; dr=1.5e-2; % dlug. i sredn rur w.g i w.z
sr=pi*(dr^2)/4; Vrg=Lrg*sr; Vrz=Lrz*sr; 
if(Fwconst) 
    vFwz=Fvwz/sr; tauFg=Lrg/vFwz; 
    nTwg1=round(tauFg/dt); nTwg=nTwg1-1; lX=4+nTwg;
    ndrg=nTwg1; ndrz=ndrg; 
else
    Fwmin=min(yFw);
...
% ..........................n.........................
Ld=length(Tzewn)-1; 
Twg0=Twg(1); Twz0=Twz(1); Ts0=Ts(1);
X(1)=T(1); X(2)=Ts(1); X(3)=Twz0; X(4:lX)=Twg0; 
% X(4:lX)  - Temperat.wody gor¹cej w rurach
% X(lX+[1:nTwg1]) - Temperat.wody zimnej w rurach
X(lX+[1:nTwg1])=Twz0; 
X0=X; 
txthet='\Theta'; nrSym=1; txDel='\Delta';
% Ograniczenie techniczne szybkoœci zmian w czasie okresu interwencji DT/dt: 
Umin=25; Umax=90; Nielin=0;
DUhReg=0.15*3600; % Max.przyrost DUr_sek*3600; 
% 3 sekundy to wymiana wody w rurkach
dUhTech=0.2*3600; % Max.przyrost w kotle DUr_sek*3600; 
...
vh=zeros(1,Ld+1); 
X(1)=T(1); X(2)=Ts(1); X(3)=Twz0; X(4:lX)=Twg0; 
X(lX+[1:nTwg1])=Twz0; X0=X; 
jestF0111=0; Nu=10; h=hPom; RegulPred;
