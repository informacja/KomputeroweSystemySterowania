...
for(nw=1:lW) %2:2) %1:lW)
    for(m=1:3) %:-1:1)
        ...
        y=[]; X=[]; t=[]; U=[]; DU=[]; y=[]; %clear y X t U DU t y U;
        ...
        for(n=1:Ld)
            ...
            if(~jestF111 && (mod(n,2500)==0 || n==Ld) && KSSlab==0)
                figure(400+m);
